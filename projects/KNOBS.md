...
| `VITASLOP_BROWSER_FASTFORWARD` | vitaslop-web/src/lib.rs:1152 | Frame to fast-forward the live loop to (`VITASLOP_BROWSER_FASTFORWARD`), unpaced. |
...
| `VITASLOP_BROWSER_HEARTBEAT_MS` | vitaslop-web/src/lib.rs:3613 | - |
...
| `VITASLOP_BROWSER_SUPERSAMPLE` | vitaslop-web/src/lib.rs:1129 | Supersample factor for the live browser render (`VITASLOP_BROWSER_SUPERSAMPLE`). |
| `VITASLOP_CALLSITES_WINDOW` | vitaslop-desktop/src/retail.rs:470 | Where the idle clock went SINCE `before` - the windowed reading. |
...
| `VITASLOP_CHAIN_DRAWS` | vitaslop-platform/src/gpu.rs:15200 | - |
...
| `VITASLOP_CLOCK_TRACE` | vitaslop-runtime/src/sched.rs:1090 | - |
...
| `VITASLOP_DRAW_RANGE` | vitaslop-platform/src/gpu.rs:501 | `VITASLOP_RTT_BG_CACHE=0` restores the OLD behaviour: a sampler bind group naming a render |
...
| `VITASLOP_DRV_KEY` | vitaslop-runtime/src/ingest/pfscrypt.rs:287 | `F00D(klicensee)` for the title, from `VITASLOP_DRV_KEY` (32 hex chars), or |
...
| `VITASLOP_DUMP_DRAW_GXP_CAP` | vitaslop-runtime/src/host.rs:13847 | - |
| `VITASLOP_DUMP_DRAW_GXP_FULL` | vitaslop-runtime/src/host.rs:13953 | - |
| `VITASLOP_DUMP_EXPORTS` | vitaslop-runtime/src/link.rs:474 | - |
...
| `VITASLOP_DUMP_FPROG` | vitaslop-runtime/src/host.rs:13735 | Diagnostic (VITASLOP_DUMP_FPROG): print the bound fragment program's sampler |
| `VITASLOP_DUMP_FUNC` | vitaslop-native/tests/retail_boot_probe.rs:317 | - |
| `VITASLOP_DUMP_GXP_BIN` | vitaslop-platform/src/gpu.rs:11381 | >>> A REFUSAL THAT DOES NOT HAND OVER THE EVIDENCE COSTS A PLAY SESSION. |
...
| `VITASLOP_DUMP_SCENES` | vitaslop-desktop/src/retail.rs:348 | Step the guest one display frame. |
| `VITASLOP_DUMP_STDOUT` | vitaslop-desktop/src/retail.rs:1508 | - |
...
| `VITASLOP_DUMP_TEX_DIR` | vitaslop-runtime/src/host.rs:13959 | - |
| `VITASLOP_DUMP_TEX_MAX_TEXELS` | vitaslop-runtime/src/host.rs:13968 | - |
...
| `VITASLOP_DUMP_VPROG` | vitaslop-runtime/src/host.rs:13543 | Diagnostic (VITASLOP_DUMP_VPROG): reflect the bound vertex program's parameter |
...
| `VITASLOP_FLAGS_WIDE_C` | vitaslop-transpiler/src/emit.rs:1271 | The A/B arm for [`emit_flags_add`]'s carry and overflow forms: `VITASLOP_FLAGS_WIDE_C=1` |
| `VITASLOP_FLAG_POISON` | vitaslop-transpiler/src/emit.rs:1229 | `VITASLOP_FLAG_POISON=0/1` - the FALSIFIER for the flag-liveness pass |
...
| `VITASLOP_FORCE_RET` | vitaslop-transpiler/src/emit.rs:1565 | Diagnostic forced return. |
| `VITASLOP_FRAME_DIGEST` | vitaslop-native/src/recipe_runner.rs:340 | - |
| `VITASLOP_FRAME_TOPUP` | vitaslop-runtime/src/host.rs:5822 | The per-flip top-up ([`VitaState::advance_time_frame`]), which is OPT-IN: |
...
| `VITASLOP_GAME_DIR` | vitaslop-runtime/src/ingest/mod.rs:136 | Test-fixture access. |
...
| `VITASLOP_GAME_ZIP` | vitaslop-runtime/src/ingest/mod.rs:116 | - |
...
| `VITASLOP_GUARD_REG` | vitaslop-transpiler/src/emit.rs:1458 | Diagnostic callee-saved-register guard. |
| `VITASLOP_GUEST_CORES` | vitaslop-runtime/src/host.rs:16148 | CPU cores a Vita gives a GAME. |
| `VITASLOP_GXM` | vitaslop-native/examples/capsule-replay.rs:196 | - |
| `VITASLOP_GXM_DEPTH_ENC` | vitaslop-platform/src/gpu.rs:2419 | Which value a later pass reads out of a render target's depth |
| `VITASLOP_GXM_NO_MULTISAMPLE` | vitaslop-platform/src/gpu.rs:5441 | A/B instrument: force every pass to ONE sample, whatever the guest asked for. |
...
| `VITASLOP_GXP_ALLOW_FIXED_FUNCTION` | vitaslop-platform/src/gpu.rs:11465 | Whether a shader pair the recompiler cannot translate may be drawn by the |
| `VITASLOP_GXP_ATTR_FILL` | vitaslop-gxp-shader/src/module.rs:770 | Bytes the driver adds to the DEFAULT uniform buffer's bound address before writing it into |
...
| `VITASLOP_GXP_CULL` | vitaslop-platform/src/gpu.rs:519 | `VITASLOP_GXP_CULL=0` restores the pre-2026-08-19b "draw both windings". |
| `VITASLOP_GXP_DEBUG` | vitaslop-platform/src/gpu.rs:11809 | Report - once per case - a GXM blend value with no exact wgpu equivalent, so the |
| `VITASLOP_GXP_DEFAULT_UNIFORM_OFFSET` | vitaslop-gxp-shader/src/module.rs:767 | Bytes the driver adds to the DEFAULT uniform buffer's bound address before writing it into |
...
| `VITASLOP_GXP_DUMP` | vitaslop-platform/src/gpu.rs:5710 | Diagnostic (`VITASLOP_GXP_KEYS=<hex>,<hex>`): recompile ONLY these shader-pair keys |
...
| `VITASLOP_GXP_EXCLUDE` | vitaslop-platform/src/gpu.rs:5714 | Pairs forced down the fixed-function path (`VITASLOP_GXP_EXCLUDE`). |
| `VITASLOP_GXP_FORCE` | vitaslop-platform/src/gpu.rs:5686 | Diagnostic (`VITASLOP_GXP_FORCE`): bind a neutral fallback texture for a sampler |
...
| `VITASLOP_GXP_INPUTS` | vitaslop-platform/src/gpu.rs:8658 | Diagnostic (`VITASLOP_GXP_INPUTS=<hex-key>[,<hex-key>]` or `=all`): print, ONCE per |
| `VITASLOP_GXP_INPUTS_DIR` | vitaslop-platform/src/gpu.rs:2535 | Whether the once-per-pair `gxp pair <key>: vprog hash ..., fprog hash ...` INDEX should be |
| `VITASLOP_GXP_INPUTS_ORDER` | vitaslop-platform/src/gpu.rs:145 | The output of a diagnostic whose own KNOB is already the gate. |
| `VITASLOP_GXP_INPUTS_VERTS` | vitaslop-platform/src/gpu.rs:8672 | Diagnostic (`VITASLOP_GXP_INPUTS=<hex-key>[,<hex-key>]` or `=all`): print, ONCE per |
| `VITASLOP_GXP_INTERP` | vitaslop-platform/src/gpu.rs:11885 | - |
| `VITASLOP_GXP_KEYCOLOR` | vitaslop-platform/src/gpu.rs:2544 | Whether the once-per-pair `gxp pair <key>: vprog hash ..., fprog hash ...` INDEX should be |
| `VITASLOP_GXP_KEYS` | vitaslop-platform/src/gpu.rs:5709 | Diagnostic (`VITASLOP_GXP_KEYS=<hex>,<hex>`): recompile ONLY these shader-pair keys |
| `VITASLOP_GXP_LIVE` | vitaslop-platform/src/gpu.rs:1304 | The guest's real vertex+fragment shaders + their draw inputs, for the GXP->WGSL |
| `VITASLOP_GXP_MIPS` | vitaslop-platform/src/gpu.rs:2815 | Whether a chain is built for a seam, ignoring the per-texture exception above. |
| `VITASLOP_GXP_NEGW` | vitaslop-platform/src/gpu.rs:5865 | How to choose the clip-`w` sign correction (`VITASLOP_GXP_NEGW`). |
| `VITASLOP_GXP_NOBLEND` | vitaslop-platform/src/gpu.rs:5704 | Diagnostic (`VITASLOP_GXP_NOBLEND`): force every recompiled pipeline to REPLACE with |
| `VITASLOP_GXP_NODEPTH` | vitaslop-platform/src/gpu.rs:5696 | Diagnostic (`VITASLOP_GXP_NODEPTH`): every recompiled draw keeps its real shading and |
| `VITASLOP_GXP_ONLY` | vitaslop-platform/src/gpu.rs:5676 | Render ONLY recompiled draws, skipping the fixed-function draw for any call that |
...
| `VITASLOP_GXP_PRECOMPILE` | vitaslop-platform/src/gpu.rs:528 | Whether a shader pair the guest's patcher names is compiled AHEAD of the draw that binds it |
| `VITASLOP_GXP_PRECOMPILE_CROSS` | vitaslop-runtime/src/host.rs:13245 | `VITASLOP_GXP_PRECOMPILE_CROSS`: for a title whose `sceGxmShaderPatcherCreateFragmentProgram` |
| `VITASLOP_GXP_PROBE` | vitaslop-gxp-shader/src/module.rs:290 | Diagnostic (`VITASLOP_GXP_PROBE=<bank><idx>[@<instr>][:f32/:bits=<hex>]`, e.g. |
| `VITASLOP_GXP_QUADS` | vitaslop-platform/src/gpu.rs:8669 | Diagnostic (`VITASLOP_GXP_INPUTS=<hex-key>[,<hex-key>]` or `=all`): print, ONCE per |
...
| `VITASLOP_GXP_RECOMPILE` | vitaslop-runtime/src/host.rs:13997 | - |
| `VITASLOP_GXP_SA` | vitaslop-platform/src/gpu.rs:9318 | Diagnostic (`VITASLOP_GXP_SA=<key>:<v/f>:<reg>=<hexword>[,...]`): replace a default-uniform |
| `VITASLOP_GXP_SA_DIRECT` | vitaslop-gxp-shader/src/link.rs:2447 | `0` restores the SA copy loop, `unroll` the constant-subscript copy - see [`resolve_sa_init`]. |
| `VITASLOP_GXP_SIZE_BANKS` | vitaslop-gxp-shader/src/link.rs:2386 | `VITASLOP_GXP_SIZE_BANKS=0` restores the pre-2026-08-20b emission - every register bank |
| `VITASLOP_GXP_SOLID` | vitaslop-platform/src/gpu.rs:5692 | Diagnostic (`VITASLOP_GXP_SOLID`): every recompiled draw outputs solid magenta with |
| `VITASLOP_GXP_VARYING_LAYOUT` | vitaslop-gxp-shader/src/link.rs:1222 | Diagnostic (`VITASLOP_GXP_VARYING_LAYOUT=<vhash>:<usage>@<lane>x<comps>,...`): plan ONE |
| `VITASLOP_GXP_VARYING_ORDER` | vitaslop-gxp-shader/src/link.rs:948 | The vertex lane order the paired FRAGMENT's declaration implies, or `None` when the two |
| `VITASLOP_GXP_VARYING_RESOLVE` | vitaslop-gxp-shader/src/link.rs:1313 | - |
...
| `VITASLOP_GXP_WGSL_DIR` | vitaslop-gxp-shader/tests/oracle.rs:779 | Link each captured vertex<->fragment PAIR into a single WGSL module and prove the linked |
| `VITASLOP_GXP_YFLIP` | vitaslop-platform/src/gpu.rs:5682 | Flip clip Y (`VITASLOP_GXP_YFLIP`, default off). |
| `VITASLOP_GXP_ZFIX` | vitaslop-platform/src/gpu.rs:5680 | Apply the GXM (GL-style, NDC z in [-1,1]) -> WebGPU (z in [0,1]) clip-depth remap |
...
| `VITASLOP_HEADLESS_FRAMES` | vitaslop-desktop/src/retail.rs:879 | Headless self-check of the retail path (NO window): load `dir`, optionally drive a |
| `VITASLOP_HEADLESS_NO_TAPS` | vitaslop-desktop/src/retail.rs:885 | Headless self-check of the retail path (NO window): load `dir`, optionally drive a |
| `VITASLOP_HEADLESS_RECIPE` | vitaslop-desktop/src/retail.rs:882 | Headless self-check of the retail path (NO window): load `dir`, optionally drive a |
| `VITASLOP_HEADLESS_SHOT_EVERY` | vitaslop-desktop/src/retail.rs:888 | Headless self-check of the retail path (NO window): load `dir`, optionally drive a |
| `VITASLOP_HEADLESS_SHOT_FROM` | vitaslop-desktop/src/retail.rs:890 | Headless self-check of the retail path (NO window): load `dir`, optionally drive a |
| `VITASLOP_HEADLESS_SHOT_TO` | vitaslop-desktop/src/retail.rs:890 | Headless self-check of the retail path (NO window): load `dir`, optionally drive a |
| `VITASLOP_HEADLESS_TIMING` | vitaslop-desktop/src/retail.rs:886 | Headless self-check of the retail path (NO window): load `dir`, optionally drive a |
...
| `VITASLOP_NEON_CACHE` | vitaslop-transpiler/src/emit.rs:2527 | Whether emitted modules hold the low NEON bank in locals across a run of vector |
...
| `VITASLOP_NO_INLINE_IMPORTS` | vitaslop-runtime/src/host.rs:6041 | >>> WHO HAS WRITTEN THE CONTEXT'S TEXTURE SLOTS, counted for the failure report above. |
...
| `VITASLOP_NO_INLINE_TEXTURE` | vitaslop-runtime/src/host.rs:6041 | >>> WHO HAS WRITTEN THE CONTEXT'S TEXTURE SLOTS, counted for the failure report above. |
...
| `VITASLOP_PAUSE_ON_BLUR` | vitaslop-desktop/src/retail.rs:1646 | Run the retail title in `dir` in a live window until the window closes or the guest |
| `VITASLOP_PEEK` | vitaslop-desktop/src/retail.rs:567 | Guest memory at `addr`, for `VITASLOP_PEEK`. |
...
| `VITASLOP_PREPARE_SPLIT` | vitaslop-platform/src/gpu.rs:4767 | Where the milliseconds INSIDE one `prepare` go, plus the bytes each phase moved. |
...
| `VITASLOP_PROMOTE_POISON` | vitaslop-transpiler/src/emit.rs:2571 | `VITASLOP_PROMOTE_POISON=<n>` - the FALSIFIER for register promotion. |
| `VITASLOP_PROMOTE_REGS` | vitaslop-native/src/threaded.rs:2324 | A concise trap description (kind + message), matching the sync `Vm`'s detail. |
...
| `VITASLOP_QUANTUM_CPU_US` | vitaslop-runtime/src/host.rs:16103 | Game-clock time charged for one [`QUANTUM_ARM`] of guest execution, in microseconds. |
| `VITASLOP_QUANTUM_FUEL` | vitaslop-native/tests/retail_boot_probe.rs:425 | - |
| `VITASLOP_REGTRACE` | vitaslop-native/src/threaded.rs:1398 | `VITASLOP_REGTRACE=<lo>-<hi>:<path>` - append the reg+flag file per block entry in |
| `VITASLOP_REGTRACE_MAX` | vitaslop-native/src/threaded.rs:1557 | `VITASLOP_REGTRACE_MAX=<n>` caps the register trace at `n` lines (0 = unbounded). |
| `VITASLOP_REGTRACE_WATCH` | vitaslop-native/src/threaded.rs:1335 | The `VITASLOP_REGTRACE_WATCH` words, formatted as ` mADDR=VALUE` fields ready to append |
| `VITASLOP_RESIDENT_GEOM` | vitaslop-platform/src/gpu.rs:5937 | Repacked vertices and expanded indices that have not changed since the renderer first |
| `VITASLOP_RESIDENT_GEOM_MB` | vitaslop-platform/src/gpu.rs:5980 | The byte budget for each of the two heaps (`VITASLOP_RESIDENT_GEOM_MB`, per heap). |
| `VITASLOP_ROUNDS_PER_FRAME` | vitaslop-native/tests/retail_boot_probe.rs:660 | - |
| `VITASLOP_RTT_BG_CACHE` | vitaslop-platform/src/gpu.rs:496 | `VITASLOP_RTT_BG_CACHE=0` restores the OLD behaviour: a sampler bind group naming a render |
| `VITASLOP_SAMPLER_NARROW` | vitaslop-runtime/src/host.rs:2844 | Whether a draw decodes only the texture units its fragment program DECLARES - see |
...
| `VITASLOP_SCHED_CORES` | vitaslop-runtime/src/sched.rs:604 | `VITASLOP_SCHED_CORES=<n>`: cap the baton to the top `n` runnable PRIORITIES, as the |
| `VITASLOP_SCHED_RR` | vitaslop-runtime/src/sched.rs:620 | `VITASLOP_SCHED_RR=1`: round-robin every runnable thread, ignoring priority. |
| `VITASLOP_SCHED_TRACE` | vitaslop-runtime/src/sched.rs:630 | `VITASLOP_SCHED_TRACE=<from>-<to>` (display frames, inclusive): print one line per |
...
| `VITASLOP_SIGNATURE_EVERY` | vitaslop-native/src/recipe_runner.rs:471 | `VITASLOP_SIGNATURE_EVERY=<n>`: print the RUNNING determinism signature every `n` stepped |
| `VITASLOP_SLOW_FRAME_US` | vitaslop-web/src/lib.rs:3581 | - |
| `VITASLOP_SNAPSHOT` | vitaslop-native/src/threaded.rs:1384 | `VITASLOP_SNAPSHOT=<hexpc>:<path>` - dump full state on first entry to block `hexpc`. |
| `VITASLOP_SNAPSHOT_BUDGET_MB` | vitaslop-runtime/src/host.rs:3301 | Byte budget for retained texture snapshots, scaled to the device |
| `VITASLOP_SNAPSHOT_DENSE` | vitaslop-native/src/threaded.rs:1494 | Dump the full guest state (all non-zero pages + r0..r15 + NZCV) to `path`, in the |
| `VITASLOP_SNAPSHOT_SKIP` | vitaslop-native/src/threaded.rs:1446 | `VITASLOP_SNAPSHOT_SKIP=<n>` - skip the first `n` entries to the snapshot block before |
| `VITASLOP_SOFTWARE` | vitaslop-desktop/src/retail.rs:1561 | - |
| `VITASLOP_SSAA` | vitaslop-platform/src/gpu.rs:12892 | Set the supersample factor: 1 (default) renders the scene straight into the caller's |
...
| `VITASLOP_TEXTURE_CHECK` | vitaslop-runtime/src/host.rs:3052 | How a retained texture snapshot is re-validated (`VITASLOP_TEXTURE_CHECK`): `scene` |
| `VITASLOP_TEX_CACHE_MB` | vitaslop-platform/src/gpu.rs:470 | The texture-cache budget in bytes: [`GAME_RESIDENT_CEILING_MB`] unless |
...
| `VITASLOP_TEX_MEMO_PER_SCENE` | vitaslop-runtime/src/host.rs:2607 | A whole DRAW's worth of snapshotted textures, by the bindings that produced it - kept |
| `VITASLOP_TEX_PAGE_READ` | vitaslop-runtime/src/host.rs:3644 | Record that this entry's bytes are current as of THIS SCENE, so a later |
| `VITASLOP_TRACE_BLOCKS` | vitaslop-native/src/threaded.rs:1310 | `VITASLOP_TRACE_FRAMES=<from>-<to>` (decimal display frames, inclusive) - print the |
...
| `VITASLOP_TRACE_FRAMES` | vitaslop-native/src/threaded.rs:1307 | `VITASLOP_TRACE_FRAMES=<from>-<to>` (decimal display frames, inclusive) - print the |
| `VITASLOP_TRACE_FUNCS` | vitaslop-native/src/threaded.rs:1231 | Bind `env.svc`. |
| `VITASLOP_TRACE_INDIRECT` | vitaslop-transpiler/src/emit.rs:1502 | Diagnostic indirect-call tracer. |
...
| `VITASLOP_TRAP_HALT` | vitaslop-transpiler/src/emit.rs:1591 | When `VITASLOP_TRAP_HALT` is set, a `Term::Halt` (a block that ran off the end of decoded |
| `VITASLOP_UNIFORM_WATCH` | vitaslop-runtime/src/vita/gxm.rs:1834 | `VITASLOP_UNIFORM_WATCH=<hex address>/<parameter name substring>[,...]`: report every |
...
| `VITASLOP_WATCH_READ` | vitaslop-transpiler/src/emit.rs:1053 | Diagnostic read watchpoint. |
| `VITASLOP_WATCH_READ_` | vitaslop-transpiler/src/emit.rs:1440 | Optional guest-PC EXCLUDE window for the read watchpoint (`VITASLOP_WATCH_READ_ |
| `VITASLOP_WATCH_READ_NZ` | vitaslop-transpiler/src/emit.rs:2963 | Emit the read-watchpoint trap check. |
| `VITASLOP_WATCH_READ_PC_EXCL` | vitaslop-transpiler/src/emit.rs:1450 | Optional guest-PC EXCLUDE window for the read watchpoint (`VITASLOP_WATCH_READ_ |
| `VITASLOP_WATCH_READ_SKIP` | vitaslop-transpiler/src/emit.rs:1154 | WASM global index of the read-watchpoint match counter, appended after the guest-PC |
| `VITASLOP_WATCH_STORE` | vitaslop-native/src/threaded.rs:1419 | `VITASLOP_REGTRACE_WATCH=<hex guest addr>[,<hex guest addr>...]` - append the WORD |
| `VITASLOP_WATCH_STORE_ARM` | vitaslop-transpiler/src/emit.rs:1613 | Store-watchpoint mode, from `VITASLOP_WATCH_STORE_MODE` (default `any`): |
