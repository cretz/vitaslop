...
|---|---|---|
| `VITASLOP_` | vitaslop-web/src/lib.rs:1255 | Set a `VITASLOP_*` knob for this run, from the page. |
| `VITASLOP_ALLOW_SOFTWARE_GPU` | vitaslop-web/src/lib.rs:519 | Whether a run may proceed on a software rasteriser (`VITASLOP_ALLOW_SOFTWARE_GPU`). |
...
| `VITASLOP_BLOCK_HIST_SEQ` | vitaslop-native/src/threaded.rs:1101 | Print the block-visit histogram gathered under `VITASLOP_BLOCK_HIST`: the `top` |
| `VITASLOP_BROWSER_FASTFORWARD` | vitaslop-web/src/lib.rs:499 | Frame to fast-forward the live loop to (`VITASLOP_BROWSER_FASTFORWARD`), unpaced. |
| `VITASLOP_BROWSER_FUEL` | vitaslop-web/src/browser_sched.rs:465 | Guest work a thread may execute before the browser preempts it, in WASMTIME FUEL UNITS |
| `VITASLOP_BROWSER_HEARTBEAT_MS` | vitaslop-web/src/lib.rs:1435 | - |
| `VITASLOP_BROWSER_QUANTUM_CALLS` | vitaslop-web/src/browser_sched.rs:84 | Host calls one guest thread may make before the browser preempts it |
| `VITASLOP_BROWSER_SUPERSAMPLE` | vitaslop-web/src/lib.rs:481 | Supersample factor for the live browser render (`VITASLOP_BROWSER_SUPERSAMPLE`). |
| `VITASLOP_CHAIN_DRAWS` | vitaslop-platform/src/gpu.rs:5179 | - |
| `VITASLOP_CHAIN_LIMIT` | vitaslop-platform/src/gpu.rs:5580 | Every offscreen target this renderer holds, as `(guest address, texture, w, h)`. |
...
| `VITASLOP_CLOCK_TRACE` | vitaslop-runtime/src/sched.rs:584 | Called when [`pick_next`](Self::pick_next) found nothing runnable. |
...
| `VITASLOP_DUMP_DRAW_GXP_CAP` | vitaslop-runtime/src/host.rs:6912 | - |
| `VITASLOP_DUMP_DRAW_GXP_FULL` | vitaslop-runtime/src/host.rs:7013 | - |
...
| `VITASLOP_DUMP_FPROG` | vitaslop-runtime/src/host.rs:6800 | Diagnostic (VITASLOP_DUMP_FPROG): print the bound fragment program's sampler |
| `VITASLOP_DUMP_FUNC` | vitaslop-native/tests/retail_boot_probe.rs:318 | - |
| `VITASLOP_DUMP_GXP_BIN` | vitaslop-runtime/src/host.rs:6844 | `VITASLOP_DUMP_GXP_BIN=<dir>`: write the raw `SceGxmProgram` blobs (the whole container - |
...
| `VITASLOP_DUMP_STDOUT` | vitaslop-desktop/src/retail.rs:720 | - |
...
| `VITASLOP_DUMP_TEX_DIR` | vitaslop-runtime/src/host.rs:7019 | - |
| `VITASLOP_DUMP_TEX_MAX_TEXELS` | vitaslop-runtime/src/host.rs:7028 | - |
...
| `VITASLOP_DUMP_VPROG` | vitaslop-runtime/src/host.rs:6639 | Diagnostic (VITASLOP_DUMP_VPROG): reflect the bound vertex program's parameter |
...
| `VITASLOP_FORCE_RET` | vitaslop-transpiler/src/emit.rs:605 | Diagnostic forced return. |
| `VITASLOP_FRAME_TOPUP` | vitaslop-runtime/src/host.rs:1915 | The per-flip top-up ([`VitaState::advance_time_frame`]), off under |
| `VITASLOP_FUEL` | vitaslop-transpiler/src/emit.rs:391 | Set the fuel interval for modules emitted on this thread after this call (0 disables). |
...
| `VITASLOP_GUARD_REG` | vitaslop-transpiler/src/emit.rs:518 | Diagnostic callee-saved-register guard. |
| `VITASLOP_GUEST_CORES` | vitaslop-runtime/src/host.rs:8191 | CPU cores a Vita gives a GAME. |
| `VITASLOP_GXM_DEPTH_ENC` | vitaslop-platform/src/gpu.rs:888 | Which value a later pass reads out of a render target's depth |
| `VITASLOP_GXM_UNIFORM_POISON` | vitaslop-runtime/src/host.rs:7398 | Diagnostic (`VITASLOP_GXM_UNIFORM_POISON=1`): fill a freshly reserved default uniform buffer |
| `VITASLOP_GXP_ALLOW_FIXED_FUNCTION` | vitaslop-platform/src/gpu.rs:3442 | Whether a shader pair the recompiler cannot translate may be drawn by the |
...
| `VITASLOP_GXP_DEBUG` | vitaslop-platform/src/gpu.rs:3673 | Report - once per case - a GXM blend value with no exact wgpu equivalent, so the |
...
| `VITASLOP_GXP_DUMP` | vitaslop-platform/src/gpu.rs:1352 | Diagnostic (`VITASLOP_GXP_KEYS=<hex>,<hex>`): recompile ONLY these shader-pair keys |
...
| `VITASLOP_GXP_EXCLUDE` | vitaslop-platform/src/gpu.rs:1356 | Pairs forced down the fixed-function path (`VITASLOP_GXP_EXCLUDE`). |
| `VITASLOP_GXP_FORCE` | vitaslop-platform/src/gpu.rs:1328 | Diagnostic (`VITASLOP_GXP_FORCE`): bind a neutral fallback texture for a sampler |
...
| `VITASLOP_GXP_INPUTS` | vitaslop-platform/src/gpu.rs:2366 | Diagnostic (`VITASLOP_GXP_INPUTS=<hex-key>[,<hex-key>]` or `=all`): print, ONCE per |
| `VITASLOP_GXP_INPUTS_DIR` | vitaslop-platform/src/gpu.rs:2496 | - |
| `VITASLOP_GXP_INTERP` | vitaslop-platform/src/gpu.rs:3719 | - |
| `VITASLOP_GXP_KEYCOLOR` | vitaslop-platform/src/gpu.rs:3101 | - |
| `VITASLOP_GXP_KEYS` | vitaslop-platform/src/gpu.rs:1351 | Diagnostic (`VITASLOP_GXP_KEYS=<hex>,<hex>`): recompile ONLY these shader-pair keys |
...
| `VITASLOP_GXP_MIPS` | vitaslop-platform/src/gpu.rs:2255 | Upload a decoded [`GxmTexture`] (linear RGBA8) to a GPU texture for the recompiler path. |
| `VITASLOP_GXP_NEGW` | vitaslop-platform/src/gpu.rs:1377 | How to choose the clip-`w` sign correction (`VITASLOP_GXP_NEGW`). |
| `VITASLOP_GXP_NOBLEND` | vitaslop-platform/src/gpu.rs:1346 | Diagnostic (`VITASLOP_GXP_NOBLEND`): force every recompiled pipeline to REPLACE with |
| `VITASLOP_GXP_NODEPTH` | vitaslop-platform/src/gpu.rs:1338 | Diagnostic (`VITASLOP_GXP_NODEPTH`): every recompiled draw keeps its real shading and |
| `VITASLOP_GXP_ONLY` | vitaslop-platform/src/gpu.rs:1318 | Render ONLY recompiled draws, skipping the fixed-function draw for any call that |
...
| `VITASLOP_GXP_RECOMPILE` | vitaslop-runtime/src/host.rs:7057 | - |
| `VITASLOP_GXP_SA` | vitaslop-platform/src/gpu.rs:2605 | Diagnostic (`VITASLOP_GXP_SA=<key>:<v/f>:<reg>=<hexword>[,...]`): replace a default-uniform |
| `VITASLOP_GXP_SOLID` | vitaslop-platform/src/gpu.rs:1334 | Diagnostic (`VITASLOP_GXP_SOLID`): every recompiled draw outputs solid magenta with |
...
| `VITASLOP_GXP_YFLIP` | vitaslop-platform/src/gpu.rs:1324 | Flip clip Y (`VITASLOP_GXP_YFLIP`, default off). |
| `VITASLOP_GXP_ZFIX` | vitaslop-platform/src/gpu.rs:1322 | Apply the GXM (GL-style, NDC z in [-1,1]) -> WebGPU (z in [0,1]) clip-depth remap |
| `VITASLOP_HEADLESS_FRAMES` | vitaslop-desktop/src/retail.rs:517 | Headless self-check of the retail path (NO window): load `dir`, optionally drive a |
| `VITASLOP_HEADLESS_NO_TAPS` | vitaslop-desktop/src/retail.rs:523 | Headless self-check of the retail path (NO window): load `dir`, optionally drive a |
| `VITASLOP_HEADLESS_RECIPE` | vitaslop-desktop/src/retail.rs:520 | Headless self-check of the retail path (NO window): load `dir`, optionally drive a |
| `VITASLOP_HEADLESS_SHOT_EVERY` | vitaslop-desktop/src/retail.rs:526 | Headless self-check of the retail path (NO window): load `dir`, optionally drive a |
| `VITASLOP_HEADLESS_TIMING` | vitaslop-desktop/src/retail.rs:524 | Headless self-check of the retail path (NO window): load `dir`, optionally drive a |
...
| `VITASLOP_PEEK` | vitaslop-desktop/src/retail.rs:348 | Guest memory at `addr`, for `VITASLOP_PEEK`. |
...
| `VITASLOP_POLL_ADDR` | vitaslop-native/src/threaded.rs:1192 | Guest address to sample after each host call, from `VITASLOP_POLL_ADDR` (hex). |
| `VITASLOP_PREPOKE` | vitaslop-native/tests/retail_boot_probe.rs:490 | - |
| `VITASLOP_QUANTUM_CPU_US` | vitaslop-runtime/src/host.rs:8168 | Game-clock time charged for one scheduler quantum of guest execution, in |
| `VITASLOP_QUANTUM_FUEL` | vitaslop-native/tests/retail_boot_probe.rs:426 | - |
| `VITASLOP_REGTRACE` | vitaslop-native/src/threaded.rs:816 | `VITASLOP_REGTRACE=<lo>-<hi>:<path>` - append the reg+flag file per block entry in |
| `VITASLOP_REGTRACE_MAX` | vitaslop-native/src/threaded.rs:976 | `VITASLOP_REGTRACE_MAX=<n>` caps the register trace at `n` lines (0 = unbounded). |
| `VITASLOP_REGTRACE_WATCH` | vitaslop-native/src/threaded.rs:832 | `VITASLOP_REGTRACE_WATCH=<hex guest addr>[,<hex guest addr>...]` - append the WORD |
...
| `VITASLOP_SNAPSHOT` | vitaslop-native/src/threaded.rs:802 | `VITASLOP_SNAPSHOT=<hexpc>:<path>` - dump full state on first entry to block `hexpc`. |
| `VITASLOP_SNAPSHOT_DENSE` | vitaslop-native/src/threaded.rs:912 | Dump the full guest state (all non-zero pages + r0..r15 + NZCV) to `path`, in the |
| `VITASLOP_SNAPSHOT_SKIP` | vitaslop-native/src/threaded.rs:864 | `VITASLOP_SNAPSHOT_SKIP=<n>` - skip the first `n` entries to the snapshot block before |
| `VITASLOP_SOFTWARE` | vitaslop-desktop/src/retail.rs:758 | - |
| `VITASLOP_SSAA` | vitaslop-platform/src/gpu.rs:4474 | Set the supersample factor: 1 (default) renders the scene straight into the caller's |
...
| `VITASLOP_STRICT_DRAWS` | vitaslop-runtime/src/render.rs:3519 | Why [`RenderSceneBuilder::build`] discarded draws from a captured scene. |
...
| `VITASLOP_TEX_CACHE_MB` | vitaslop-platform/src/gpu.rs:933 | Upper bound on the cross-frame texture caches before they are cleared wholesale |
| `VITASLOP_TRACE_BLOCKS` | vitaslop-transpiler/src/emit.rs:585 | Diagnostic per-basic-block execution tracer. |
...
| `VITASLOP_TRACE_FUNCS` | vitaslop-native/src/threaded.rs:724 | Bind `env.svc`. |
| `VITASLOP_TRACE_INDIRECT` | vitaslop-transpiler/src/emit.rs:562 | Diagnostic indirect-call tracer. |
...
| `VITASLOP_TRAP_HALT` | vitaslop-transpiler/src/emit.rs:631 | When `VITASLOP_TRAP_HALT` is set, a `Term::Halt` (a block that ran off the end of decoded |
...
| `VITASLOP_WASM_NAMES` | vitaslop-transpiler/src/emit.rs:288 | When `VITASLOP_WASM_NAMES` is set, emit a wasm `name` custom section labelling |
| `VITASLOP_WATCH_` | vitaslop-transpiler/src/emit.rs:474 | Number of matching store-watchpoint hits to skip before trapping (`VITASLOP_WATCH_ |
...
| `VITASLOP_WATCH_READ` | vitaslop-transpiler/src/emit.rs:260 | Diagnostic read watchpoint. |
| `VITASLOP_WATCH_READ_` | vitaslop-transpiler/src/emit.rs:500 | Optional guest-PC EXCLUDE window for the read watchpoint (`VITASLOP_WATCH_READ_ |
| `VITASLOP_WATCH_READ_NZ` | vitaslop-transpiler/src/emit.rs:1415 | Emit the read-watchpoint trap check. |
| `VITASLOP_WATCH_READ_PC_EXCL` | vitaslop-transpiler/src/emit.rs:510 | Optional guest-PC EXCLUDE window for the read watchpoint (`VITASLOP_WATCH_READ_ |
| `VITASLOP_WATCH_READ_SKIP` | vitaslop-transpiler/src/emit.rs:336 | WASM global index of the read-watchpoint match counter, appended after the guest-PC |
| `VITASLOP_WATCH_STORE` | vitaslop-native/src/threaded.rs:837 | `VITASLOP_REGTRACE_WATCH=<hex guest addr>[,<hex guest addr>...]` - append the WORD |
| `VITASLOP_WATCH_STORE_ARM` | vitaslop-transpiler/src/emit.rs:653 | Store-watchpoint mode, from `VITASLOP_WATCH_STORE_MODE` (default `any`): |
| `VITASLOP_WATCH_STORE_LOG` | vitaslop-transpiler/src/emit.rs:656 | `VITASLOP_WATCH_STORE_LOG` - LOG each store to the watched address (the storing |
| `VITASLOP_WATCH_STORE_MODE` | vitaslop-transpiler/src/emit.rs:646 | Store-watchpoint mode, from `VITASLOP_WATCH_STORE_MODE` (default `any`): |
| `VITASLOP_WATCH_STORE_NZ` | vitaslop-transpiler/src/emit.rs:669 | `VITASLOP_WATCH_STORE_LOG` - LOG each store to the watched address (the storing |
| `VITASLOP_WATCH_STORE_SKIP` | vitaslop-transpiler/src/emit.rs:342 | WASM global index of the store-watchpoint match counter (appended after `TP_GLOBAL`). |
