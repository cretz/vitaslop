...
| `VITASLOP_BROWSER_HEARTBEAT_MS` | vitaslop-web/src/lib.rs:2393 | - |
...
| `VITASLOP_CHAIN_DRAWS` | vitaslop-platform/src/gpu.rs:8576 | - |
...
| `VITASLOP_DEBUG_CAPTURE` | vitaslop-web/src/lib.rs:2415 | - |
...
| `VITASLOP_DUMP_DRAW_GXP_CAP` | vitaslop-runtime/src/host.rs:8373 | - |
| `VITASLOP_DUMP_DRAW_GXP_FULL` | vitaslop-runtime/src/host.rs:8474 | - |
...
| `VITASLOP_DUMP_FPROG` | vitaslop-runtime/src/host.rs:8261 | Diagnostic (VITASLOP_DUMP_FPROG): print the bound fragment program's sampler |
| `VITASLOP_DUMP_FUNC` | vitaslop-native/tests/retail_boot_probe.rs:319 | - |
| `VITASLOP_DUMP_GXP_BIN` | vitaslop-runtime/src/host.rs:8305 | `VITASLOP_DUMP_GXP_BIN=<dir>`: write the raw `SceGxmProgram` blobs (the whole container - |
...
| `VITASLOP_DUMP_TEX_DIR` | vitaslop-runtime/src/host.rs:8480 | - |
| `VITASLOP_DUMP_TEX_MAX_TEXELS` | vitaslop-runtime/src/host.rs:8489 | - |
...
| `VITASLOP_DUMP_VPROG` | vitaslop-runtime/src/host.rs:8100 | Diagnostic (VITASLOP_DUMP_VPROG): reflect the bound vertex program's parameter |
...
| `VITASLOP_GUEST_CORES` | vitaslop-runtime/src/host.rs:9939 | CPU cores a Vita gives a GAME. |
| `VITASLOP_GXM_DEPTH_ENC` | vitaslop-platform/src/gpu.rs:1632 | Which value a later pass reads out of a render target's depth |
| `VITASLOP_GXM_NO_MULTISAMPLE` | vitaslop-platform/src/gpu.rs:2738 | A/B instrument: force every pass to ONE sample, whatever the guest asked for. |
| `VITASLOP_GXM_UNIFORM_POISON` | vitaslop-runtime/src/host.rs:8884 | Diagnostic (`VITASLOP_GXM_UNIFORM_POISON=1`): fill a freshly reserved default uniform buffer |
| `VITASLOP_GXP_ALLOW_FIXED_FUNCTION` | vitaslop-platform/src/gpu.rs:6352 | Whether a shader pair the recompiler cannot translate may be drawn by the |
...
| `VITASLOP_GXP_DEBUG` | vitaslop-platform/src/gpu.rs:6611 | Report - once per case - a GXM blend value with no exact wgpu equivalent, so the |
...
| `VITASLOP_GXP_DUMP` | vitaslop-platform/src/gpu.rs:2971 | Diagnostic (`VITASLOP_GXP_KEYS=<hex>,<hex>`): recompile ONLY these shader-pair keys |
...
| `VITASLOP_GXP_EXCLUDE` | vitaslop-platform/src/gpu.rs:2975 | Pairs forced down the fixed-function path (`VITASLOP_GXP_EXCLUDE`). |
| `VITASLOP_GXP_FORCE` | vitaslop-platform/src/gpu.rs:2947 | Diagnostic (`VITASLOP_GXP_FORCE`): bind a neutral fallback texture for a sampler |
...
| `VITASLOP_GXP_INPUTS` | vitaslop-platform/src/gpu.rs:4554 | Diagnostic (`VITASLOP_GXP_INPUTS=<hex-key>[,<hex-key>]` or `=all`): print, ONCE per |
| `VITASLOP_GXP_INPUTS_DIR` | vitaslop-platform/src/gpu.rs:1729 | Whether the once-per-pair `gxp pair <key>: vprog hash ..., fprog hash ...` INDEX should be |
| `VITASLOP_GXP_INPUTS_ORDER` | vitaslop-platform/src/gpu.rs:41 | The output of a diagnostic whose own KNOB is already the gate. |
| `VITASLOP_GXP_INPUTS_VERTS` | vitaslop-platform/src/gpu.rs:1728 | Whether the once-per-pair `gxp pair <key>: vprog hash ..., fprog hash ...` INDEX should be |
| `VITASLOP_GXP_INTERP` | vitaslop-platform/src/gpu.rs:6657 | - |
| `VITASLOP_GXP_KEYCOLOR` | vitaslop-platform/src/gpu.rs:5755 | - |
| `VITASLOP_GXP_KEYS` | vitaslop-platform/src/gpu.rs:2970 | Diagnostic (`VITASLOP_GXP_KEYS=<hex>,<hex>`): recompile ONLY these shader-pair keys |
| `VITASLOP_GXP_LIVE` | vitaslop-platform/src/gpu.rs:791 | The guest's real vertex+fragment shaders + their draw inputs, for the GXP->WGSL |
| `VITASLOP_GXP_MIPS` | vitaslop-platform/src/gpu.rs:1926 | Whether [`upload_gxp_texture`] builds a mip chain for this seam. |
| `VITASLOP_GXP_NEGW` | vitaslop-platform/src/gpu.rs:3071 | How to choose the clip-`w` sign correction (`VITASLOP_GXP_NEGW`). |
| `VITASLOP_GXP_NOBLEND` | vitaslop-platform/src/gpu.rs:2965 | Diagnostic (`VITASLOP_GXP_NOBLEND`): force every recompiled pipeline to REPLACE with |
| `VITASLOP_GXP_NODEPTH` | vitaslop-platform/src/gpu.rs:2957 | Diagnostic (`VITASLOP_GXP_NODEPTH`): every recompiled draw keeps its real shading and |
| `VITASLOP_GXP_ONLY` | vitaslop-platform/src/gpu.rs:2937 | Render ONLY recompiled draws, skipping the fixed-function draw for any call that |
...
| `VITASLOP_GXP_PAIRS` | vitaslop-platform/src/gpu.rs:1715 | Whether the once-per-pair `gxp pair <key>: vprog hash ..., fprog hash ...` INDEX should be |
...
| `VITASLOP_GXP_RECOMPILE` | vitaslop-runtime/src/host.rs:8518 | - |
| `VITASLOP_GXP_SA` | vitaslop-platform/src/gpu.rs:4975 | Diagnostic (`VITASLOP_GXP_SA=<key>:<v/f>:<reg>=<hexword>[,...]`): replace a default-uniform |
| `VITASLOP_GXP_SOLID` | vitaslop-platform/src/gpu.rs:2953 | Diagnostic (`VITASLOP_GXP_SOLID`): every recompiled draw outputs solid magenta with |
...
| `VITASLOP_GXP_YFLIP` | vitaslop-platform/src/gpu.rs:2943 | Flip clip Y (`VITASLOP_GXP_YFLIP`, default off). |
| `VITASLOP_GXP_ZFIX` | vitaslop-platform/src/gpu.rs:2941 | Apply the GXM (GL-style, NDC z in [-1,1]) -> WebGPU (z in [0,1]) clip-depth remap |
...
| `VITASLOP_QUANTUM_CPU_US` | vitaslop-runtime/src/host.rs:9916 | Game-clock time charged for one full [`QUANTUM_FUEL`] of guest execution, in |
...
| `VITASLOP_SSAA` | vitaslop-platform/src/gpu.rs:7461 | Set the supersample factor: 1 (default) renders the scene straight into the caller's |
...
| `VITASLOP_TEX_CACHE_MB` | vitaslop-platform/src/gpu.rs:1781 | Upper bound on the cross-frame texture caches before they are cleared wholesale |
...
| `VITASLOP_WATCH_STORE_LOG` | vitaslop-runtime/src/capture.rs:419 | GUEST ADDRESS the bytes above were read from, or 0 when there is no bound buffer. |
