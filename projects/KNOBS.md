...
|---|---|---|
| `VITASLOP_A` | vitaslop-frontend/src/settings.rs:234 | - |
...
| `VITASLOP_ALLOW_SOFTWARE_GPU` | vitaslop-web/src/lib.rs:1148 | Whether a run may proceed on a software rasteriser (`VITASLOP_ALLOW_SOFTWARE_GPU`). |
...
| `VITASLOP_B` | vitaslop-frontend/src/settings.rs:234 | - |
...
| `VITASLOP_BLOCK_HIST_SEQ` | vitaslop-native/src/threaded.rs:1663 | Print the block-visit histogram gathered under `VITASLOP_BLOCK_HIST`: the `top` |
| `VITASLOP_BROWSER_FASTFORWARD` | vitaslop-web/src/lib.rs:1128 | Frame to fast-forward the live loop to (`VITASLOP_BROWSER_FASTFORWARD`), unpaced. |
...
| `VITASLOP_BROWSER_HEARTBEAT_MS` | vitaslop-web/src/lib.rs:3589 | - |
...
| `VITASLOP_BROWSER_SUPERSAMPLE` | vitaslop-web/src/lib.rs:1105 | Supersample factor for the live browser render (`VITASLOP_BROWSER_SUPERSAMPLE`). |
| `VITASLOP_CALLSITES_WINDOW` | vitaslop-desktop/src/retail.rs:462 | Where the idle clock went SINCE `before` - the windowed reading. |
...
| `VITASLOP_CHAIN_DRAWS` | vitaslop-platform/src/gpu.rs:15193 | - |
...
| `VITASLOP_DEBUG_CAPTURE` | vitaslop-frontend/src/settings.rs:138 | The `VITASLOP_*` map a run of these settings is configured with: the base set, |
...
| `VITASLOP_DRV_KEY` | vitaslop-runtime/src/ingest/pfscrypt.rs:288 | `F00D(klicensee)` for the title, from `VITASLOP_DRV_KEY` (32 hex chars), or |
...
| `VITASLOP_DUMP_DRAW_GXP_CAP` | vitaslop-runtime/src/host.rs:13841 | - |
| `VITASLOP_DUMP_DRAW_GXP_FULL` | vitaslop-runtime/src/host.rs:13947 | - |
| `VITASLOP_DUMP_EXPORTS` | vitaslop-runtime/src/link.rs:464 | - |
...
| `VITASLOP_DUMP_FPROG` | vitaslop-runtime/src/host.rs:13729 | Diagnostic (VITASLOP_DUMP_FPROG): print the bound fragment program's sampler |
...
| `VITASLOP_DUMP_SCENES` | vitaslop-desktop/src/retail.rs:340 | Step the guest one display frame. |
| `VITASLOP_DUMP_STDOUT` | vitaslop-desktop/src/retail.rs:1505 | - |
...
| `VITASLOP_DUMP_TEX_DIR` | vitaslop-runtime/src/host.rs:13953 | - |
| `VITASLOP_DUMP_TEX_MAX_TEXELS` | vitaslop-runtime/src/host.rs:13962 | - |
...
| `VITASLOP_DUMP_VPROG` | vitaslop-runtime/src/host.rs:13536 | Diagnostic (VITASLOP_DUMP_VPROG): reflect the bound vertex program's parameter |
...
| `VITASLOP_FRAME_TOPUP` | vitaslop-runtime/src/host.rs:5828 | The per-flip top-up ([`VitaState::advance_time_frame`]), which is OPT-IN: |
...
| `VITASLOP_GUEST_CORES` | vitaslop-runtime/src/host.rs:16143 | CPU cores a Vita gives a GAME. |
...
| `VITASLOP_GXP_PRECOMPILE_CROSS` | vitaslop-runtime/src/host.rs:13254 | `VITASLOP_GXP_PRECOMPILE_CROSS`: for a title whose `sceGxmShaderPatcherCreateFragmentProgram` |
...
| `VITASLOP_GXP_RECOMPILE` | vitaslop-runtime/src/host.rs:13991 | - |
...
| `VITASLOP_GXP_SA_DIRECT` | vitaslop-gxp-shader/src/link.rs:2448 | `0` restores the SA copy loop, `unroll` the constant-subscript copy - see [`resolve_sa_init`]. |
| `VITASLOP_GXP_SIZE_BANKS` | vitaslop-gxp-shader/src/link.rs:2387 | `VITASLOP_GXP_SIZE_BANKS=0` restores the pre-2026-08-20b emission - every register bank |
...
| `VITASLOP_GXP_VARYING_LAYOUT` | vitaslop-gxp-shader/src/link.rs:1220 | Diagnostic (`VITASLOP_GXP_VARYING_LAYOUT=<vhash>:<usage>@<lane>x<comps>,...`): plan ONE |
| `VITASLOP_GXP_VARYING_ORDER` | vitaslop-gxp-shader/src/link.rs:946 | The vertex lane order the paired FRAGMENT's declaration implies, or `None` when the two |
| `VITASLOP_GXP_VARYING_RESOLVE` | vitaslop-gxp-shader/src/link.rs:1312 | - |
...
| `VITASLOP_HEADLESS_FRAMES` | vitaslop-desktop/src/retail.rs:875 | Headless self-check of the retail path (NO window): load `dir`, optionally drive a |
| `VITASLOP_HEADLESS_NO_TAPS` | vitaslop-desktop/src/retail.rs:881 | Headless self-check of the retail path (NO window): load `dir`, optionally drive a |
| `VITASLOP_HEADLESS_RECIPE` | vitaslop-desktop/src/retail.rs:878 | Headless self-check of the retail path (NO window): load `dir`, optionally drive a |
| `VITASLOP_HEADLESS_SHOT_EVERY` | vitaslop-desktop/src/retail.rs:884 | Headless self-check of the retail path (NO window): load `dir`, optionally drive a |
| `VITASLOP_HEADLESS_SHOT_FROM` | vitaslop-desktop/src/retail.rs:886 | Headless self-check of the retail path (NO window): load `dir`, optionally drive a |
| `VITASLOP_HEADLESS_SHOT_TO` | vitaslop-desktop/src/retail.rs:886 | Headless self-check of the retail path (NO window): load `dir`, optionally drive a |
| `VITASLOP_HEADLESS_TIMING` | vitaslop-desktop/src/retail.rs:882 | Headless self-check of the retail path (NO window): load `dir`, optionally drive a |
...
| `VITASLOP_NAME` | vitaslop-desktop/src/shell.rs:715 | - |
...
| `VITASLOP_NO_INLINE_IMPORTS` | vitaslop-runtime/src/host.rs:6047 | >>> WHO HAS WRITTEN THE CONTEXT'S TEXTURE SLOTS, counted for the failure report above. |
...
| `VITASLOP_NO_INLINE_TEXTURE` | vitaslop-runtime/src/host.rs:6047 | >>> WHO HAS WRITTEN THE CONTEXT'S TEXTURE SLOTS, counted for the failure report above. |
...
| `VITASLOP_PAUSE_ON_BLUR` | vitaslop-desktop/src/retail.rs:1643 | Run the retail title in `dir` in a live window until the window closes or the guest |
| `VITASLOP_PEEK` | vitaslop-desktop/src/retail.rs:559 | Guest memory at `addr`, for `VITASLOP_PEEK`. |
| `VITASLOP_PERF` | vitaslop-native/src/perf.rs:43 | Is perf accounting on (`VITASLOP_PERF` set)? Read once and cached. |
| `VITASLOP_PERF_CONSOLE` | vitaslop-web/src/lib.rs:1161 | Whether the per-window performance report is also written to the browser CONSOLE |
...
| `VITASLOP_POLL_ADDR` | vitaslop-native/src/threaded.rs:1754 | Guest address to sample after each host call, from `VITASLOP_POLL_ADDR` (hex). |
...
| `VITASLOP_PRESENT_PROBE` | vitaslop-web/src/lib.rs:559 | Reads back WHAT WE PRESENTED, when `VITASLOP_PRESENT_PROBE` asks for it. |
...
| `VITASLOP_PROMOTE_REGS` | vitaslop-native/src/threaded.rs:2317 | A concise trap description (kind + message), matching the sync `Vm`'s detail. |
...
| `VITASLOP_QUANTUM_CPU_US` | vitaslop-runtime/src/host.rs:16098 | Game-clock time charged for one [`QUANTUM_ARM`] of guest execution, in microseconds. |
...
| `VITASLOP_SAMPLER_NARROW` | vitaslop-runtime/src/host.rs:2840 | Whether a draw decodes only the texture units its fragment program DECLARES - see |
...
| `VITASLOP_SLOW_FRAME_US` | vitaslop-web/src/lib.rs:3557 | - |
...
| `VITASLOP_SNAPSHOT_BUDGET_MB` | vitaslop-runtime/src/host.rs:3296 | Byte budget for retained texture snapshots, scaled to the device |
...
| `VITASLOP_SOFTWARE` | vitaslop-desktop/src/retail.rs:1558 | - |
...
| `VITASLOP_SWITCH_WHY` | vitaslop-transpiler/src/lower.rs:1073 | Whether the table-branch diagnostic is on for this address |
...
| `VITASLOP_TEXTURE_CHECK` | vitaslop-runtime/src/host.rs:3047 | How a retained texture snapshot is re-validated (`VITASLOP_TEXTURE_CHECK`): `scene` |
...
| `VITASLOP_TEX_MEMO_PER_SCENE` | vitaslop-runtime/src/host.rs:2603 | A whole DRAW's worth of snapshotted textures, by the bindings that produced it - kept |
| `VITASLOP_TEX_PAGE_READ` | vitaslop-runtime/src/host.rs:3640 | Record that this entry's bytes are current as of THIS SCENE, so a later |
...
| `VITASLOP_UNIFORM_WATCH` | vitaslop-runtime/src/vita/gxm.rs:1819 | `VITASLOP_UNIFORM_WATCH=<hex address>/<parameter name substring>[,...]`: report every |
...
| `VITASLOP_VERTEX_INTERN` | vitaslop-runtime/src/host.rs:2825 | A cheap, allocation-free fingerprint of a vertex stream, for [`TextureSnapshots:: |
| `VITASLOP_VPK` | vitaslop-runtime/src/ingest/stream.rs:994 | A homebrew VPK (`VITASLOP_VPK=<file>`): probed as `vpk`, imported as files + a |
| `VITASLOP_WASM_INDICES` | vitaslop-native/src/threaded.rs:2371 | Rewrite `<wasm function N>` in a trap backtrace to name the GUEST function it is. |
...
| `VITASLOP_WATCH_READ_NZ` | vitaslop-transpiler/src/emit.rs:2968 | Emit the read-watchpoint trap check. |
...
| `VITASLOP_X` | vitaslop-frontend/src/settings.rs:214 | Parse a `NAME=VALUE` per line knobs box into a map. |
...
| `VITASLOP_Y` | vitaslop-frontend/src/settings.rs:215 | Parse a `NAME=VALUE` per line knobs box into a map. |
