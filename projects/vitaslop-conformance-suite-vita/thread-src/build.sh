#!/bin/bash
# Reproduce the threading artifacts (thread.elf, thread.velf) from thread.c.
#
...
vita-elf-create thread.elf thread.velf

echo "OK: thread.elf ($(stat -c%s thread.elf) B), thread.velf ($(stat -c%s thread.velf) B)"
